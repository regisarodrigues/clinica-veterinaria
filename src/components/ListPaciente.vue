<script setup>
defineEmits(['atualizar-paciente', 'deletar-paciente']);
...
<template>
  <div class="mx-5 my-10 bg-white shadow-md px-5 py-10 rounded-xl">
    ...
    <p class="font-bold mb-3 text-gray-700 uppercase">
      Nome Pet:
      <span class="font-normal normal-case"> {{ paciente.pet }} </span>
    </p>

    <p class="font-bold mb-3 text-gray-700 uppercase">
      Propietário:
      <span class="font-normal normal-case"> {{ paciente.proprietario }} </span>
    </p>
    ...
    <div class="grid md:grid-cols-2 gap-5 mt-10">
      <button
        ...
        @click="$emit('atualizar-paciente', paciente.id)"
      >
        Editar
      </button>
      ...
      <button
        ...
        @click="$emit('deletar-paciente', paciente.id)"
      >
        Deletar
      </button>
    </div>
